...
class GnuPlot {
    ...
    protected function sendInit()
    {
        ...
        if ($this->xlabel) {
            $this->sendCommand('set xlabel "'.$this->xlabel.'"'); # offset 0,-2');
        }

        if ($this->timeFormat) {
#            $this->sendCommand('set bmargin 8');
            $this->sendCommand('set xdata time');
            $this->sendCommand('set timestamp');
            $this->sendCommand('set timefmt "'.$this->timeFormat.'"');
#            $this->sendCommand('set xtics rotate by 45 offset -6,-3');
#            $this->sendCommand('set key on bmargin');
            $this->sendCommand('set key top left');
#            $this->sendCommand('set xtics rotate by 90 offset 0, -5');
